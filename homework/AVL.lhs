...
The goal this homework is to implement a purely functional version of AVL trees in Haskell. I
f you are unfamiliar with this data structure, the wikipedia page is a good start.

As usual, you should work on [this file][avlhs].
A solution to this homework will eventually be available [here][avl-sol].
...
This definition is similar to that of standard binary trees, the only difference is that nodes
store the height of the tree at that point.
...
The height of a tree is the maximum distance from a node any leaf below it.
...
right subtree of the node. An invariant of AVL trees is that the balance factor must be between -1 and 1.
...
the tree is balanced. In this problem, you\'ll need to define quickcheck properties for those invariants.
