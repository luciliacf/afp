...
[datatypes-sol]: datatypes-sol.lhs

Note: this is the completed version of lecture [Lec4][datatypes]. The [lhs version][datatypes-sol] of this file is available.
...
have downsides.  If you use `type Day = String`
...
> sumOfIntListNE (ICons i rest) = i + sumOfIntListNE rest
> 
...
> foldTree b f Empty = b
> foldTree b f (Branch x l r) = f x (foldTree b f l) (foldTree b f r)

Now, let's reimplement infixOrder:

> infixOrder' :: Tree a -> [a]
> infixOrder' = foldTree [] (\x ll rl -> ll ++ [x] ++ rl)
...
> prefixOrder' :: Tree a -> [a]
> prefixOrder' = foldTree [] (\x ll rl -> [x] ++ ll ++ rl)

> postfixOrder' :: Tree a -> [a]
> postfixOrder' = foldTree [] (\x ll rl -> ll ++ rl ++ [x])
