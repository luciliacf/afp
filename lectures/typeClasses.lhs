...
---
[typeclasseslhs]: typeclasses.lhs

*Note:* this is the stubbed version of module Type Classes. You may download the [lhs version][typeclasseslhs]
of this module.
...
>   Empty          == Empty          = True
>   Branch x t1 t2 == Branch y u1 u2 = x==y && t1==u1 && t2==u2
>   _              == _              = False
 ...
 * serialization \-\- converting to a string, binary format, JSON, or other format
 * hashCode \-\- for indexing into hashtables
 * truthiness (bool conversion) \-\- if null then `...` else `...`
 * other conversions \-\- toInteger
 * iterators \-\- traversing or visiting data structures.
 * comparison \-\- equality
...
hard-to-understand code and unpredictable behavior. For example, see this
talk by [Gary Bernhardt from CodeMesh 2012][gary].

[gary]: https://www.destroyallsoftware.com/talks/wat
...
ghci> \x -> (x,x)
  ...
  <interactive>:1:1:
      No instance for (Show (t0 -> (t0, t0)))
        arising from a use of `print'
      Possible fix: add an instance declaration for (Show (t0 -> (t0, t0)))
      In a stmt of an interactive GHCi command: print it
~~~~~~~
 ...
 * transitivity for `(<)`
 * `(<)` is opposite of `(>=)`
 * compatibility between `compare` and `(<)`
 * etc
...
We already observed that we could use the `[a..b]` list syntax on
...
-------
  ...
  * Miran Lipovaca, *Learn You a Haskell for Great Good!*, chapter 7.
  
  * Graham Hutton, *Programming in Haskell* , 2nd Ed., chapter 8.
